...
LIB_API int loadModel(char* mtcnn_model,char* insightface_params,char * insightface_json) {
	recognizer.loadModel(mtcnn_model, insightface_params, insightface_json);
}
...
LIB_API char * computeDistance(char * base_emb, char * target_emb){
	...
	try{
		...
		cJSON_AddNumberToObject(result, "distence", distance);
		resultJson = cJSON_PrintUnformatted(result);
		return resultJson;
	}
	catch (const std::exception&){
		...
		cJSON_AddNumberToObject(result, "distence", -1);
		resultJson = cJSON_PrintUnformatted(result);
		return resultJson;
	}
